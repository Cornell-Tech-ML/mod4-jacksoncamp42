...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert pytest.approx(1 - sigmoid_a) == sigmoid(-a)
    assert pytest.approx(sigmoid(0)) == 0.5
    ...
    if a < -epsilon:
        ...
        assert pytest.approx(sigmoid(a)) == 0.5
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert pytest.approx(mul(a, b)) == mul(b, a)
...
def test_other(a: float, b: float) -> None:
    ...
    """Test that relu(max(a, b)) == max(relu(a), relu(b))"""
    # TODO: Implement for Task 0.2.
    left = relu(max(a, b))
    right = max(relu(a), relu(b))
    assert pytest.approx(left) == right
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    # Sum of ls1 plus sum of ls2
    sum1 = minitorch.operators.sum(ls1) + minitorch.operators.sum(ls2)
    # Sum of element-wise addition of ls1 and ls2
...
def test_sum(ls: List[float]) -> None:
    ...
    assert_close(minitorch.operators.sum([]), 0.0)
